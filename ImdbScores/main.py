...
data
test = data
json = json.loads(data[0][-1])

countries = [j['displayText'] for j in json]
rates = [j['aggregateRating'] for j in json]
counts = [j['totalVoteCount'] for j in json]

plt.bar(ocuntries, rates)import pandas as pd
import numpy as np
import pymysql
import requests
import json
